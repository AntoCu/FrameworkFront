...
const bddPlats = [
  { "Pays": "Albanie", "Plat": "Tavë Kosi" },
  { "Pays": "Allemagne", "Plat": "Schnitzel" },
  { "Pays": "Autriche", "Plat": "Strudel aux pommes" },
  { "Pays": "Belgique", "Plat": "Carbonnade" },
  { "Pays": "Bielorussie", "Plat": "Bortsch" },
  { "Pays": "Bosnie-Herzegovine", "Plat": "Cevapi" },
  { "Pays": "Bulgarie", "Plat": "Banitsa" },
  { "Pays": "Croatie", "Plat": "Sarma" },
  { "Pays": "Danemark", "Plat": "Stegt Flaesk" },
  { "Pays": "Espagne", "Plat": "Paella" },
  { "Pays": "Estonie", "Plat": "Silgusoust" },
  { "Pays": "Finlande", "Plat": "Poronkäristys" },
  { "Pays": "France", "Plat": "Boeuf Bourguignon" },
  { "Pays": "Grece", "Plat": "Moussaka" },
  { "Pays": "Hongrie", "Plat": "Goulash" },
  { "Pays": "Irlande", "Plat": "Irish Stew" },
  { "Pays": "Islande", "Plat": "Hakarl" },
  { "Pays": "Italie", "Plat": "Pizza" },
  { "Pays": "Kosovo", "Plat": "Flia" },
  { "Pays": "Lettonie", "Plat": "Karbonade" },
  { "Pays": "Lituanie", "Plat": "Capelinai" },
  { "Pays": "Luxembourg", "Plat": "KuddelFleck" },
  { "Pays": "Macedoine", "Plat": "Tavče gravče" },
  { "Pays": "Moldavie", "Plat": "Mamaliga" },
  { "Pays": "Montenegro", "Plat": "Kacamak" },
  { "Pays": "Norvege", "Plat": "Fårikål " },
  { "Pays": "Pays-Bas", "Plat": "Erwtensoep" },
  { "Pays": "Pologne", "Plat": "Bigos" },
  { "Pays": "Portugal", "Plat": "Bacalhau" },
  { "Pays": "Republique Tcheque", "Plat": "Vepřo knedlo zelo" },
  { "Pays": "Royaume Unis", "Plat": "Fish & Chips" },
  { "Pays": "Serbie", "Plat": "Cevapcici" },
  { "Pays": "Slovaquie", "Plat": "Bryndzové halušky" },
  { "Pays": "Slovenie", "Plat": "Ajdovi Zganci" },
  { "Pays": "Suede", "Plat": "Ärtsoppa" },
  { "Pays": "Suisse", "Plat": "Rösti" },
  { "Pays": "Ukraine", "Plat": "Pampushky" }
];
const paysJouables = bddPlats.filter((item) => item.Plat !== "?");
...
export default function App() {
  ...
  const [imageSrc, setImageSrc] = useState("/imagesFood/default.jpg");

  let displayPlat, displayScoreV, displayScoreE;

  useEffect(() => {
    displayPlat = document.getElementById("nom-plat");
    displayScoreV = document.getElementById("score-valide");
    displayScoreE = document.getElementById("score-erreur");

    lancerNouveauRound();
  }, []); 

  function normaliserNomFichier(plat) {
    return plat
      .toLowerCase()
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
  ...
  function verifierReponse(element) {
    if (!paysEnCours) return;

    if (element.id === paysEnCours.Pays) {
      element.classList.add("correct");
      setScoreV((prev) => prev + 1);
  ...
  function lancerNouveauRound() {
    const elementsColores = document.querySelectorAll(".correct, .wrong");
    elementsColores.forEach((el) => {
      el.classList.remove("correct", "wrong");
    });

    const index = Math.floor(Math.random() * paysJouables.length);
    const nouveauPays = paysJouables[index];

    setPaysEnCours(nouveauPays);

    if (displayPlat) displayPlat.innerText = nouveauPays.Plat;

    const nomBase = normaliserNomFichier(nouveauPays.Plat);
    setImageSrc(`/imagesFood/${nomBase}.jpg`);
  }
  ...
  return (
    <div className="bg-orange-50 min-h-screen font-sans text-gray-800">
      ...
      <div className="flex flex-row gap-8 justify-center">
        <div className="ml-10 bg-white max-w-2xl p-6 rounded-2xl shadow-lg border-2 border-orange-100 h-fit">
          <p className="text-gray-400 text-center text-sm uppercase tracking-wider mb-2">
            Plat à trouver :
          </p>

          <h2 id="nom-plat" className="text-center text-4xl font-black text-gray-800">
            {paysEnCours?.Plat || "Chargement..."}
          </h2>
          ...
          <img
            ...
            onError={(e) => {
              ...
              e.target.src = "/imagesFood/default.jpg";
            }}
            className="w-64 h-64 object-cover mx-auto rounded-xl mt-4 shadow-md border border-orange-100"
          />

          <div className="flex justify-center gap-8 mt-6 font-bold text-lg">
            <div className="text-green-600 bg-green-100 px-4 py-2 rounded-lg">
              Trouvés : <span id="score-valide">{scoreV}</span>
            </div>
            <div className="text-red-500 bg-red-100 px-4 py-2 rounded-lg">
              Erreurs : <span id="score-erreur">{scoreE}</span>
            </div>
          </div>
        </div>

        <main className="w-full max-w-5xl overflow-hidden rounded-2xl shadow-lg border-2 border-orange-100">
          <div className="w-full max-w-5xl bg-white p-4 rounded-xl shadow-xl overflow-hidden">
            <svg
              id="europe-map"
              baseprofile="tiny" fill="#ececec" height="684" stroke="black"
              stroke-linecap="round" stroke-linejoin="round" stroke-width=".1" version="1.2" viewbox="0 0 1000 684"
              width="1000" xmlns="http://www.w3.org/2000/svg"
              onClick={(e) => {
                if (e.target.tagName === "path") {
                  verifierReponse(e.target);
                }
              }}
            >
              <path
              ...
              <path
                ...
                id="Bosnie-Herzegovine" name="Bosnia and Herzegovina">
              </path>
              <path
              ...
              <path
                ...
                id="Republique Tcheque" name="Czech Republic">
              </path>
              <path
              ...
              <path
                ...
                id="Royaume Unis" name="United Kingdom">
              </path>
              <path
